...
// ─── Body-system helpers (shared with FlashCard) ──────────────────────────────────────────
...
// Compute top body systems represented in the queue, sorted by frequency.
function topSystems(
  cards: DueCard[],
  max = 4
...
function SessionSummary({
  ...
  const extraSystemCount = Math.max(0, new Set(sessionCards.map((c) => c.bodySystem).filter(Boolean)).size - systems.length);

  if (sessionCount === 0) {
    return (
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: spacing['2xl'] }}>
        <Text variant="headline" align="center" color={colors.text}>
          All caught up!
        </Text>
  ...
  return (
    ...
    >
      {/* Header */}
      <Text variant="title" color={colors.textSecondary} align="center">
        Ready to study
      </Text>

      {/* Hero summary — Playfair Display */}
      <View style={{ alignItems: 'center', gap: spacing.xs }}>
        ...
        <Text variant="headline" color={colors.textSecondary} align="center"
          style={{ fontFamily: fontFamilies.displayRegular, fontSize: fontSizes.xl }}
        >
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: spacing.xs }}>
            {systems.map(({ system, count }) => (
              ...
              >
                ...
                >
                  <Text style={{ fontSize: 9, color: colors.surface, fontFamily: fontFamilies.bodyBold }}>
                    {count}
                  </Text>
                </View>
              </View>
...
function ActiveSession({
  ...
  // Fade-swap animation between cards.
  ...
  // Advance queue after the FlashCard's confidence submission succeeds.
  const handleReviewed = useCallback((quality: number) => {
    // Brief pause so the user sees "Review saved ✓" before the card swaps.
...
interface ReviewSessionProps {
  ...
  onDismiss?: () => void;
  /** Pre-applied filter set from SessionFilter. */
  filters?: QueueFilters;
}

export function ReviewSession({ onDismiss, filters = EMPTY_FILTERS }: ReviewSessionProps) {
  const { colors, spacing } = useTheme();
  ...
  if (queue.status === 'error') {
    return (
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: spacing.xl, gap: spacing.lg }}>
        <Text variant="title" color={colors.error} align="center">
